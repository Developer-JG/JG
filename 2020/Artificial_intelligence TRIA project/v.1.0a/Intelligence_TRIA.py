...
timezone = datetime.datetime.now(timezone(city))
    ...
    for i in range(len(command_result)):
        try:
            if command_result[i] == '그' and command_result[i + 1] == '그' and command_result[i + 2] == '저' and command_result[i + 3] == '께' or \
                ...
                set_timezone = timezone - datetime.timedelta(days = 3)
        except IndexError:
            pass

        try:
            if command_result[i] == '그' and command_result[i + 1] == '저' and command_result[i + 2] == '께':
                set_timezone = timezone - datetime.timedelta(days = 2)
        except IndexError:
            pass

        try:
            if command_result[i] == '어' and command_result[i + 1] == '제':
                set_timezone = timezone - datetime.timedelta(days = 1)
        except IndexError:
            pass
        ...
        try:
            if command_result[i] == '그' and command_result[i + 1] == '글' and command_result[i + 2] == '피':
                set_timezone = timezone + datetime.timedelta(days = 4)
        except IndexError:
            pass
